{ lib, config, options, pkgs, ... }: {
  options.lh.services.gitea = {
    ...
    traefikIntegration = {
      ...
      certResolver = lib.mkOption {
        type = lib.types.str;
        default = if config.lh.services.gitea.traefikIntegration.enable then
      ...
      middlewares = lib.mkOption {
        ...
        default = [ ];
        description =
          "A list of middlewares to apply to the Gitea Traefik router";
      };
    };
  ...
  in lib.mkIf config.lh.services.gitea.enable (persistenceDef // {
    assertions = [{
      assertion = config.lh.services.gitea.mailer.enable
        -> config.lh.services.gitea.mailer.passwordFile != null;
      ...
      services = {
        gitea = {
          serviceConfig = {
            ...
            ReadWritePaths = [ "/var/lib/gitea" "/run/gitea" ];
            ...
            BindPaths =
              [ "/var/lib/gitea:/var/lib/gitea" "/run/gitea:/run/gitea" ];
          };
    ...
    lh.services = {
      ...
      traefik.dynamicConfig =
        lib.mkIf config.lh.services.gitea.traefikIntegration.enable {
          http.routers.gitea = {
            rule = "Host(`${config.lh.services.gitea.domain}`)";
            entryPoints = [ "websecure" ];
            service = "gitea";
            middlewares = [ "securityheaders" ]
              ++ config.lh.services.gitea.traefikIntegration.middlewares;
            tls = {
          ...
          http.services.gitea.loadBalancer.servers =
            [{ url = "unix+http:/run/gitea/gitea.sock"; }];
        };
